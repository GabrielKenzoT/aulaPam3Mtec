...
import {View,Text,ActivityIndicator,StyleSheet,Image,Dimensions,SafeAreaView,TouchableOpacity,Alert, ScrollView} from 'react-native';
...
export default function App() {
  ...
  async function uploadImage() {
      ...
      try {
        const response = await fetch("http://10.239.0.125/imagem/upload.php", {
          method: 'POST',
          body: formData,
          headers: {
...
//divisão para eu nao me perder
  ...
  useEffect(() => {
    async function carregarImagens() {
      try {
        const response = await fetch('http://10.239.0.125/imagem/lista_imagens.php');
