...
const API_URL = "http://10.239.0.124/chatDentista";

const EMOJIS_POSSIVEIS = ["👨‍⚕️", "👩‍⚕️", "⚕️", "🦷", "👨🏻‍⚕️"];
const EMOJI_PADRAO = "😷";
...
export default function App() {
  ...
  const carregarMensagens = async () => {
    try {
      ...
      setMensagens(msgs); // Sem reverse - FlatList usará inverted
  ...
  if (!nomeConfirmado) {
    return (
      <View style={styles.loginContainer}>
        <Text style={styles.titulo}>Dentista Kenzo TiraKare</Text>
        <Image style={styles.Imagem} source={require("./assets/Dentista.png")} />
        <Text style={styles.subtitulo}>Digite o nome: </Text>
        <TextInput
          style={styles.input}
  ...
  return (
    ...
    >
      <Text style={styles.header}> Bem Vindo a TiraKare {usuario}</Text>

      <FlatList
        ref={flatListRef}
        data={mensagens}
        keyExtractor={(item) => item.id.toString()}
        inverted={true} // Aqui aplica a inversão visual da lista
        renderItem={({ item }) => {
